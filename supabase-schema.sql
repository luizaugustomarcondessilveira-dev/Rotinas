...
-- ==============================================================================
-- Este script cria todas as tabelas com Row Level Security (RLS) para garantir
-- o isolamento multi-inquilino (cada família só tem acesso aos seus próprios dados).
--
-- Como executar:
...
-- ------------------------------------------------------------------------------
-- 3. TABELA: CONFIGURAÇÕES DA FAMÍLIA (family_settings)
-- ------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.family_settings (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    family_id UUID NOT NULL REFERENCES public.families(id) ON DELETE CASCADE,
    family_name TEXT NOT NULL DEFAULT 'Rotinas da Família',
    pin_code_hash TEXT NOT NULL, -- SHA-256 hash da senha parental (nunca texto puro)
...
-- ------------------------------------------------------------------------------
-- 4. TABELA: MEMBROS / PERFIS DA FAMÍLIA (members)
-- ------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.members (
    ...
    pin_hash TEXT NOT NULL, -- SHA-256 hash do PIN individual
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
...
-- ------------------------------------------------------------------------------
-- 5. TABELA: ATIVIDADES & CATÁLOGO DE ROTINAS (routine_tasks)
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 6. TABELA: LOJA DE INCENTIVOS & PRÊMIOS (reward_items)
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 7. TABELA: PONTUAÇÃO & EXTRATO DE TRANSAÇÕES (transactions)
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 8. TABELA: COMPROMISSOS (appointments)
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 9. TABELA: NOTIFICAÇÕES (notifications)
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 11. ROW LEVEL SECURITY (RLS) - ISOLAMENTO SEGURO POR FAMÍLIA
-- ------------------------------------------------------------------------------
...
-- Função auxiliar segura para identificar a família do usuário conectado
...
DECLARE
  ...
  v_req_header TEXT;
BEGIN
  -- Se o usuário estiver autenticado no Supabase Auth
  ...
  -- Se foi enviado o cabeçalho x-family-id
  BEGIN
...
CREATE POLICY "families_all_policy" ON public.families
  FOR ALL
  USING (
    auth_user_id = auth.uid()
    OR id = public.current_family_id()
    OR auth.role() = 'anon'
  )
  WITH CHECK (
    auth_user_id = auth.uid()
...
-- Políticas para 'family_settings'
DROP POLICY IF EXISTS "settings_family_isolation" ON public.family_settings;
CREATE POLICY "settings_family_isolation" ON public.family_settings
  FOR ALL
  USING (
    family_id = public.current_family_id()
    OR family_id IN (SELECT id FROM public.families WHERE auth_user_id = auth.uid())
    OR auth.role() = 'anon'
  )
  WITH CHECK (
    family_id = public.current_family_id()
...
CREATE POLICY "members_family_isolation" ON public.members
  FOR ALL
  USING (
    family_id = public.current_family_id()
    OR family_id IN (SELECT id FROM public.families WHERE auth_user_id = auth.uid())
...
-- Políticas para 'routine_tasks'
DROP POLICY IF EXISTS "tasks_family_isolation" ON public.routine_tasks;
CREATE POLICY "tasks_family_isolation" ON public.routine_tasks
  FOR ALL
  USING (
    family_id = public.current_family_id()
    OR family_id IN (SELECT id FROM public.families WHERE auth_user_id = auth.uid())
...
-- Políticas para 'reward_items'
DROP POLICY IF EXISTS "rewards_family_isolation" ON public.reward_items;
CREATE POLICY "rewards_family_isolation" ON public.reward_items
  FOR ALL
  USING (
    family_id = public.current_family_id()
    OR family_id IN (SELECT id FROM public.families WHERE auth_user_id = auth.uid())
...
CREATE POLICY "tx_family_isolation" ON public.transactions
  FOR ALL
  USING (
    family_id = public.current_family_id()
    OR family_id IN (SELECT id FROM public.families WHERE auth_user_id = auth.uid())
...
CREATE POLICY "appointments_family_isolation" ON public.appointments
  FOR ALL
  USING (
    family_id = public.current_family_id()
    OR family_id IN (SELECT id FROM public.families WHERE auth_user_id = auth.uid())
...
CREATE POLICY "notifications_family_isolation" ON public.notifications
  FOR ALL
  USING (
    family_id = public.current_family_id()
    OR family_id IN (SELECT id FROM public.families WHERE auth_user_id = auth.uid())
...
-- Inserção de uma família inicial padrão caso a tabela esteja vazia
